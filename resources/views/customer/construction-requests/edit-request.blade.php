<x-customer-layout>
    @php
        ...
        $productFieldsDisabled = $isProductRequest && !$request->product?->is_resizable;
    @endphp

    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('customer.construction-requests.show', $request->id) }}" class="p-2 bg-green-100 hover:bg-green-200 rounded-lg transition-colors">
                <svg class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                </svg>
            </a>
            <h2 class="font-bold text-2xl text-gray-900">Modifier la demande</h2>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="bg-white rounded-xl border-2 border-green-100 overflow-hidden shadow-lg">
                        ...
                        <div class="p-6">
                            <div class="flex gap-6">
                                <div class="w-40 h-40 flex-shrink-0 rounded-xl overflow-hidden bg-gradient-to-br from-green-100 to-emerald-100">
                                ...
                                <div class="flex-1 space-y-3">
                                    <div>
                                        <p class="text-xs uppercase tracking-wide font-semibold text-emerald-600 mb-1">{{ $isProductRequest ? 'Demande produit' : 'Demande concept' }}</p>
                                    ...
                                    @if($subjectCategory)
                                        <span class="inline-flex items-center px-3 py-1 bg-green-100 text-green-700 rounded-full text-sm font-semibold">{{ $subjectCategory->name }}</span>
                                    @endif
                                    <div class="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
                                        Toute modification remettra la demande en attente et les anciennes offres seront écartées pour laisser place à de nouvelles propositions.
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="space-y-6">
                    <div class="bg-white rounded-xl border-2 border-green-100 overflow-hidden shadow-lg">
                        <div class="p-6 bg-gradient-to-r from-green-50 to-emerald-50 border-b border-green-100">
                            <h3 class="text-xl font-bold text-gray-900">Modifier la demande</h3>
                        </div>
                        <div class="p-6">
                            <form action="{{ route('customer.construction-requests.update', $request->id) }}" method="POST" class="space-y-6">
                                @csrf
                                @method('PUT')
                                <input type="hidden" name="viewer_state_json" value="{{ $request->viewer_state_json }}">
                                ...
                                @if($productFieldsDisabled)
                                    <div class="p-4 rounded-xl border border-amber-200 bg-amber-50 text-amber-800">
                                        ...
                                        <p class="text-sm mt-1">La taille et les dimensions sont fixées par le producteur pour cette demande.</p>
                                    </div>
                                @endif
                                ...
                                <div class="grid grid-cols-1 gap-4">
                                    <div>
                                        ...
                                        <select id="requested_size" name="requested_size" @if(!$productFieldsDisabled) required @endif @disabled($productFieldsDisabled) class="w-full border-2 border-green-200 rounded-lg p-3 focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white disabled:bg-gray-100 disabled:text-gray-500 disabled:cursor-not-allowed">
                                    ...
                                    <div>
                                        ...
                                        <input id="requested_length" name="requested_length" type="number" step="0.01" min="0" value="{{ old('requested_length', $dimensions['length'] ?? '') }}" @disabled($productFieldsDisabled) class="w-full border-2 border-green-200 rounded-lg p-3 focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white disabled:bg-gray-100 disabled:text-gray-500 disabled:cursor-not-allowed">
                                    </div>
                                    <div>
                                        <label for="requested_width" class="block text-sm font-semibold text-gray-700 mb-2">Largeur</label>
                                        <input id="requested_width" name="requested_width" type="number" step="0.01" min="0" value="{{ old('requested_width', $dimensions['width'] ?? '') }}" @disabled($productFieldsDisabled) class="w-full border-2 border-green-200 rounded-lg p-3 focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white disabled:bg-gray-100 disabled:text-gray-500 disabled:cursor-not-allowed">
                                    </div>
                                    <div>
                                        <label for="requested_height" class="block text-sm font-semibold text-gray-700 mb-2">Hauteur</label>
                                        <input id="requested_height" name="requested_height" type="number" step="0.01" min="0" value="{{ old('requested_height', $dimensions['height'] ?? '') }}" @disabled($productFieldsDisabled) class="w-full border-2 border-green-200 rounded-lg p-3 focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white disabled:bg-gray-100 disabled:text-gray-500 disabled:cursor-not-allowed">
                                    </div>
                                    <div>
                                ...
                                <div>
                                    ...
                                    <textarea id="message" name="message" rows="4" class="w-full border-2 border-green-200 rounded-lg p-3 focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white resize-none">{{ old('message', $request->message) }}</textarea>
                                </div>

                                <div>
                                    <label for="customer_notes" class="block text-sm font-semibold text-gray-700 mb-2">Notes supplémentaires</label>
                                    <textarea id="customer_notes" name="customer_notes" rows="5" class="w-full border-2 border-green-200 rounded-lg p-3 focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white resize-none">{{ old('customer_notes', $request->customer_notes) }}</textarea>
                                </div>

                                <button type="submit" class="w-full inline-flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl font-bold hover:from-emerald-600 hover:to-teal-600 transition-all duration-300 shadow-lg hover:shadow-xl">
    ...
    <script>
        ...
        const editRequestForm = document.querySelector('form[action*="/construction-requests/"]');
        if (editRequestForm) {
            editRequestForm.addEventListener('submit', async function (event) {
                event.preventDefault();

                const submitButton = event.submitter;
                const viewerStateInput = editRequestForm.querySelector('input[name="viewer_state_json"]');
                ...
                if (typeof window.showAppLoader === 'function') {
                    window.showAppLoader('Mise à jour de votre demande en cours...');
                }
