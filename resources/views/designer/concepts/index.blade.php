        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gradient-to-br from-white via-purple-50/30 to-indigo-50/30 overflow-hidden shadow-xl sm:rounded-2xl border border-purple-100">
                <div class="p-6">
                    ...
                    <div class="mb-6" x-data="realtimeSearch({ endpoint: '{{ route('designer.concepts.index') }}' })">
                        <form method="GET" action="{{ route('designer.concepts.index') }}" class="flex flex-wrap gap-4 items-end">
                            <div class="flex-1 min-w-[200px] relative">
                                ...
                                <input type="text" name="search" value="{{ request('search') }}" x-model="searchQuery" @input="onSearchInput()" placeholder="Rechercher un concept en temps réel..."
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        @foreach($concepts as $concept)
                                            <tr class="hover:bg-purple-50/50 transition-colors" data-search-name="{{ strtolower($concept->name) }}">
                                                ...
                                                <td class="px-6 py-4">
                                                    <div class="flex items-center gap-3">
                                                        <a href="{{ route('designer.concepts.show', $concept) }}" class="w-16 h-16 rounded-lg bg-gradient-to-br from-purple-100 to-indigo-100 flex items-center justify-center overflow-hidden flex-shrink-0 hover:ring-2 hover:ring-purple-300 transition" title="Voir le concept">
