<x-constructor-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route($routePrefix . '.products.show', $product) }}" class="px-6 py-3 bg-gray-200 text-gray-700 rounded-xl font-semibold hover:bg-gray-300 transition-all">
                Voir le produit
            </a>
        </div>
    </x-slot>
    ...
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        <div class="bg-gradient-to-br from-white via-teal-50/30 to-cyan-50/30 overflow-hidden shadow-xl sm:rounded-2xl border border-teal-100">
            <div class="p-8">
                ...
                <p class="text-gray-600 mb-8">Choisissez une ou plusieurs options (sous-matériaux) pour chaque matériau sélectionné sur ce produit.</p>

                <form action="{{ route($routePrefix . '.products.save-personalize', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    @csrf

                    @forelse($product->metals as $metal)
                        <div class="bg-white/80 rounded-xl p-6 border-2 border-teal-100 shadow-sm">
                            ...
                            <p class="text-sm text-gray-500 mb-4">Sélectionnez un ou plusieurs sous-matériaux pour ce matériau</p>

                            @if($metal->metalOptions->count() > 0)
                                @php $selectedIds = $selectedOptionsByMetal->get($metal->id) ?? collect(); @endphp
                                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                    @foreach($metal->metalOptions as $option)
                                        @php $isSelected = $selectedIds->contains((int) $option->id); @endphp
                            ...
                            @php $customRows = $customOptionsByMetal->get($metal->id, collect()); @endphp
                            <div class="mt-6 pt-5 border-t border-teal-100">
                                    ...
                                    @foreach($customRows as $index => $custom)
                                        <div class="custom-row grid grid-cols-1 md:grid-cols-12 gap-3 p-3 rounded-xl border border-teal-100 bg-teal-50/30">
                                            ...
                                            <div class="md:col-span-4">
                                                <input type="text" name="custom_options[{{ $metal->id }}][{{ $index }}][name]" value="{{ $custom->name }}" placeholder="Nom" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" />
                                            </div>
                                            <div class="md:col-span-3">
                                                <input type="text" name="custom_options[{{ $metal->id }}][{{ $index }}][ref]" value="{{ $custom->ref }}" placeholder="Réf" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" />
                                            </div>
                                            <div class="md:col-span-3">
                                            ...
                                            <div class="md:col-span-1 flex items-center">@if($custom->image_url)<img src="{{ $custom->image_url }}" class="w-10 h-10 rounded object-cover border border-gray-200" />@endif</div>
                                            <div class="md:col-span-1 flex items-center justify-end"><button type="button" class="remove-custom-row text-red-500 hover:text-red-700 text-sm">Supprimer</button></div>
                                        </div>
                                    @endforeach
                                </div>
                                <button type="button" data-metal-id="{{ $metal->id }}" class="add-custom-row mt-3 inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-teal-100 text-teal-700 hover:bg-teal-200 text-sm font-semibold">
                                    <span>+</span><span>Ajouter un sous-matériau</span>
                                </button>
                            </div>
                        </div>
                    @empty
                        <p class="text-gray-500">Aucun matériau associé à ce produit.</p>
                    @endforelse

                    @if($product->metals->count() > 0)
                        <div class="flex justify-end gap-4 pt-6 border-t border-teal-100">
                            <a href="{{ route($routePrefix . '.products.show', $product) }}" class="px-6 py-3 bg-gray-200 text-gray-700 rounded-xl font-semibold hover:bg-gray-300">Annuler</a>
                            <button type="submit" class="px-8 py-3 bg-gradient-to-r from-teal-500 to-cyan-500 text-white rounded-xl font-bold hover:from-teal-600 hover:to-cyan-600 shadow-lg">Enregistrer la personnalisation</button>
                        </div>
                    @endif
                </form>
            </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.querySelectorAll('.add-custom-row').forEach((btn) => {
                btn.addEventListener('click', function () {
                    ...
                    row.innerHTML = `
                        <div class="md:col-span-4"><input type="text" name="custom_options[${metalId}][${index}][name]" placeholder="Nom" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" /></div>
                        <div class="md:col-span-3"><input type="text" name="custom_options[${metalId}][${index}][ref]" placeholder="Réf" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" /></div>
                        <div class="md:col-span-4"><input type="file" name="custom_options[${metalId}][${index}][image]" accept="image/*" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white" /></div>
                        <div class="md:col-span-1 flex items-center justify-end"><button type="button" class="remove-custom-row text-red-500 hover:text-red-700 text-sm">Supprimer</button></div>
                    `;
