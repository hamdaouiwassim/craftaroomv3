                ...
                <div class="p-8">
                    ...
                    <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                            ...
                            <div class="space-y-6">
                                ...
                                <div>
                                    ...
                                    <textarea name="description" id="description" rows="4"
                                              class="w-full border-2 border-purple-200 rounded-lg p-3 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 bg-white">{{ old('description', $category->description) }}</textarea>
                                    @error('description')
                                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select name="type" id="type" onchange="toggleParentCategory()" required
                                            class="w-full border-2 border-purple-200 rounded-lg p-3 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 bg-white">
                                        <option value="main" {{ old('type', $category->type) == 'main' ? 'selected' : '' }}>Principale</option>
                                        <option value="sub" {{ old('type', $category->type) == 'sub' ? 'selected' : '' }}>Secondaire</option>
                                    </select>
                                    @error('type')
                                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                    @enderror
                                ...
                                <div id="parent-category-group" style="display: {{ old('type', $category->type) == 'sub' ? 'block' : 'none' }};">
                                    <label for="category_id" class="block text-sm font-semibold text-gray-700 mb-2">Catégorie parente</label>
                                    <select name="category_id" id="category_id"
                                            class="w-full border-2 border-purple-200 rounded-lg p-3 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 bg-white">
                                        <option value="">Sélectionner une catégorie</option>
                                        @foreach($categories as $cat)
                                            <option value="{{ $cat->id }}" {{ old('category_id', $category->category_id) == $cat->id ? 'selected' : '' }}>
                                                {{ $cat->name }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="bg-white/60 backdrop-blur-sm rounded-xl p-6 border border-purple-100 shadow-sm">
                            ...
                            @if($category->icon)
                                <div class="mb-4">
                                    <label class="block text-sm font-semibold text-gray-700 mb-2">Icône actuelle</label>
                                    <div class="w-20 h-20 rounded-lg bg-gradient-to-br from-purple-100 to-indigo-100 overflow-hidden">
                            ...
                            <div>
                                <label for="icon" class="block text-sm font-semibold text-gray-700 mb-2">Télécharger une nouvelle icône</label>
