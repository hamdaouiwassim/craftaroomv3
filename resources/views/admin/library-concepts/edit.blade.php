                ...
                <div class="max-w-6xl mx-auto">
                    ...
                    <form action="{{ route('admin.library-concepts.update', $concept) }}" method="POST" class="mt-4" id="concept-form"
                        ...
                        <div class="mb-8">
                            ...
                            <div class="bg-white/60 backdrop-blur-sm rounded-xl p-5 border border-teal-100 shadow-sm md:col-span-2">
                                <label for="style_type" class="inline-flex items-center gap-3 cursor-pointer">
                                    <input type="checkbox" name="style_type" id="style_type" value="artisant" {{ old('style_type', $concept->style_type) === 'artisant' ? 'checked' : '' }}
                                        class="w-5 h-5 text-teal-600 border-2 border-teal-300 rounded focus:ring-2 focus:ring-teal-500">
                                    <span class="text-sm font-semibold text-gray-700">Artisant</span>
                                </label>
                                        ...
                                        <div class="flex flex-wrap gap-3 mb-4">
                                            @foreach($concept->photos as $photo)
                                                <div class="relative group">
                                                    ...
                                                    <button type="button"
                                                            class="absolute -top-1 -right-1 p-1 bg-red-500 hover:bg-red-600 text-white rounded-full shadow"
                                                            aria-label="Supprimer"
                                                            onclick="if (confirm('Supprimer cette photo ?')) document.getElementById('delete-photo-{{ $photo->id }}').submit();">
                    ...
                    @foreach($concept->photos as $photo)
                        <form id="delete-photo-{{ $photo->id }}" action="{{ route('admin.library-concepts.delete-photo', [$concept, $photo]) }}" method="POST" class="hidden">
                            @csrf
                            @method('DELETE')
                        </form>
                    @endforeach
