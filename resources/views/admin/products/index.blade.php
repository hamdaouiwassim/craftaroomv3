<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between" x-data="{ addProductModalOpen: false }" @open-add-product-modal.window="addProductModalOpen = true">
            ...
            <button type="button" @click="addProductModalOpen = true" class="flex items-center gap-2 bg-gradient-to-r from-teal-500 via-cyan-500 to-blue-500 text-white px-6 py-3 rounded-xl font-bold hover:from-teal-600 hover:via-cyan-600 hover:to-blue-600 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105">
            ...
            <template x-teleport="body">
                <div x-show="addProductModalOpen" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-modal="true" role="dialog">
                    <div class="flex min-h-full items-center justify-center p-4">
                        ...
                        <div x-show="addProductModalOpen" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl max-w-2xl w-full p-6 border border-teal-100">
                            ...
                            <div class="grid gap-4 sm:grid-cols-1">
                                <a href="{{ route('admin.products.create') }}" class="flex items-center gap-4 p-4 rounded-xl border-2 border-teal-200 hover:border-teal-400 hover:bg-teal-50/50 transition-all group">
                                    <div class="p-3 bg-teal-100 rounded-xl group-hover:bg-teal-200 transition-colors">
                                        <svg class="w-8 h-8 text-teal-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>
                                    </div>
                                    <div class="flex-1">
                                ...
                                <a href="{{ route('admin.concepts.select', ['source' => 'designer']) }}" class="flex items-center gap-4 p-4 rounded-xl border-2 border-teal-200 hover:border-teal-400 hover:bg-teal-50/50 transition-all group">
                                ...
                                <a href="{{ route('admin.concepts.select', ['source' => 'library']) }}" class="flex items-center gap-4 p-4 rounded-xl border-2 border-teal-200 hover:border-teal-400 hover:bg-teal-50/50 transition-all group">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gradient-to-br from-white via-teal-50/30 to-cyan-50/30 overflow-hidden shadow-xl sm:rounded-2xl border border-teal-100">
                <div class="p-6">
                    ...
                    <div class="mb-6" x-data="realtimeSearch({ endpoint: '{{ route('admin.products.index') }}' })">
                        <div class="flex gap-4">
                            <div class="flex-1 relative">
                                ...
                                <input type="text" 
                                       ...
                                       class="block w-full pl-12 pr-12 py-3 border-2 border-teal-200 rounded-xl shadow-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500 bg-white text-gray-900 placeholder-gray-400">
                                
                                <!-- Loading spinner -->
                                <div x-show="isLoading" class="absolute inset-y-0 right-0 pr-4 flex items-center">
                                    <svg class="animate-spin h-5 w-5 text-teal-500" fill="none" viewBox="0 0 24 24">
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        @foreach($products as $product)
                                            <tr class="hover:bg-teal-50/50 transition-colors" data-search-name="{{ strtolower($product->name) }}">
                                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                                    {{ $loop->iteration }}
                                                </td>
                                                <td class="px-6 py-4">
                                                    <div class="flex items-center gap-3">
                                                        <a href="{{ route('admin.products.show', $product) }}" class="w-16 h-16 rounded-lg bg-gradient-to-br from-teal-100 to-cyan-100 flex items-center justify-center overflow-hidden flex-shrink-0 hover:ring-2 hover:ring-cyan-300 transition" title="Voir le produit">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <span class="text-sm font-bold bg-gradient-to-r from-teal-600 to-cyan-600 bg-clip-text text-transparent">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    <div class="flex items-center justify-end gap-2">
                                                        <a href="{{ route('admin.products.show', $product) }}" class="p-2 text-cyan-600 hover:bg-cyan-50 rounded-lg transition-colors" title="Voir">
                                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                                            </svg>
                                                        </a>
                                                        <a href="{{ route('admin.products.edit', $product) }}" class="p-2 text-teal-600 hover:bg-teal-50 rounded-lg transition-colors" title="Modifier">
                                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @else
                        <div class="text-center py-16 bg-white rounded-xl border border-teal-100">
                            <div class="inline-block p-4 bg-gradient-to-br from-teal-100 to-cyan-100 rounded-full mb-4">
                            ...
                            <a href="{{ route('admin.products.create') }}" class="inline-flex items-center gap-2 bg-gradient-to-r from-teal-500 via-cyan-500 to-blue-500 text-white px-6 py-3 rounded-xl font-bold hover:from-teal-600 hover:via-cyan-600 hover:to-blue-600 transition-all duration-300 shadow-lg hover:shadow-xl">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                                </svg>
