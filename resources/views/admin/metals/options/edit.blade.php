        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500">Metal: {{ $metal->name }}</p>
                </div>
            </div>
            <a href="{{ route('admin.metals.show', $metal) }}" class="px-4 py-2 rounded-xl bg-gray-100 text-gray-700 font-semibold hover:bg-gray-200 transition-all">Back</a>
        </div>
    </x-slot>
            ...
            <div class="bg-white/90 backdrop-blur border border-blue-100 shadow-xl sm:rounded-2xl p-8">
                ...
                <form method="POST" action="{{ route('admin.metals.options.update', [$metal, $option]) }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="ref" name="ref" type="text" class="mt-1 block w-full" value="{{ old('ref', $option->ref) }}" placeholder="e.g. MET-A1" />
                        <x-input-error :messages="$errors->get('ref')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="name" value="Sub metal name" />
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" value="{{ old('name', $option->name) }}" required />
                    ...
                    <div class="flex justify-end gap-3">
                        <a href="{{ route('admin.metals.show', $metal) }}" class="px-6 py-3 rounded-xl bg-gray-100 text-gray-700 font-semibold hover:bg-gray-200 transition-all">Cancel</a>
                        <button type="submit" class="px-6 py-3 rounded-xl bg-gradient-to-r from-blue-500 to-blue-accent text-white font-semibold shadow-lg hover:shadow-xl transition-all">Update Option</button>
                    </div>
                </form>
            </div>
        </div>
